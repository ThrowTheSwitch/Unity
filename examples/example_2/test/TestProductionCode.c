...
TEST(ProductionCode, FindFunction_WhichIsBroken_ShouldReturnZeroIfItemIsNotInList_WhichWorksEvenInOurBrokenCode)
{
  ...
  TEST_ASSERT__EQUAL(0, FindFunction_WhichIsBroken(78));
  TEST_ASSERT__EQUAL(0, FindFunction_WhichIsBroken(2));
  TEST_ASSERT__EQUAL(0, FindFunction_WhichIsBroken(33));
  TEST_ASSERT__EQUAL(0, FindFunction_WhichIsBroken(999));
  TEST_ASSERT__EQUAL(0, FindFunction_WhichIsBroken(-1));
}
...
TEST(ProductionCode, FindFunction_WhichIsBroken_ShouldReturnTheIndexForItemsInList_WhichWillFailBecauseOurFunctionUnderTestIsBroken)
{
  ...
  TEST_ASSERT__EQUAL(1, FindFunction_WhichIsBroken(34));

  // Notice the rest of these didn't get a chance to run because the line above failed.
  // Unit tests abort each test function on the first sign of trouble.
  // Then NEXT test function runs as normal.
  TEST_ASSERT__EQUAL(8, FindFunction_WhichIsBroken(8888));
}

TEST(ProductionCode, FunctionWhichReturnsLocalVariable_ShouldReturnTheCurrentCounterValue)
{
    //This should be true because setUp set this up for us before this test
    TEST_ASSERT__EQUAL__HEX(0x5a5a, FunctionWhichReturnsLocalVariable());

    //This should be true because we can still change our answer
    Counter = 0x1234;
    TEST_ASSERT__EQUAL__HEX(0x1234, FunctionWhichReturnsLocalVariable());
}
