...
void test_ShouldCallMockInitAndVerifyFunctionsForEachTest(void)
{
    ...
    TEST_ASSERT__EQUAL__MESSAGE(Unity.NumberOfTests,     mockMock_Init_Counter,    "Mock Init Should Be Called Once Per Test Started");
    TEST_ASSERT__EQUAL__MESSAGE(passesOrIgnores,         mockMock_Verify_Counter,  "Mock Verify Should Be Called Once Per Test Passed");
    TEST_ASSERT__EQUAL__MESSAGE(Unity.NumberOfTests - 1, mockMock_Destroy_Counter, "Mock Destroy Should Be Called Once Per Test Completed");
    TEST_ASSERT__EQUAL__MESSAGE(0,                       CMockMemFreeFinalCounter, "Mock MemFreeFinal Should Not Be Called Until End");
}
