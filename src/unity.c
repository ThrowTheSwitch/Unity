...
static void
UnityPrintDecimalAndNumberWithLeadingZeros(UNITY_INT32 fraction_part,
                                           UNITY_INT32 divisor) {
  UNITY_OUTPUT_CHAR('.');
  while (divisor > 0) {
...
/* If rounds up && remainder 0.5 && result odd && below cutoff for double
 * precision issues */
#define ROUND_TIES_TO_EVEN(orig, num_int, num)                                 \
  if (num_int > (num) && (num) - (num_int - 1) <= 0.5 && (num_int & 1) == 1 && \
      orig < 1e22)                                                             \
...
/* Printing floating point numbers is hard. Some goals of this implementation:
 works for embedded
 * systems, floats or doubles, and has a reasonable format. The key paper in
 this area,
 * 'How to Print Floating-Point Numbers Accurately' by Steele & White, shows an
 approximation by
 * scaling called Dragon 2. This code uses a similar idea. The other core
 algorithm uses casts and
 * floating subtraction to give exact remainders after the decimal, to be scaled
 into an integer.
 * Extra trailing 0's are excluded. The output defaults to rounding to nearest,
 ties to even. You
 * can enable rounding ties away from zero. Note: UNITY_DOUBLE param can typedef
 to float or double

 * The old version required compiling in snprintf. For reference, with a similar
 format as now:
 *  char buf[19];
 *  if (number > 4294967296.0 || -number > 4294967296.0) snprintf(buf, sizeof
 buf, "%.8e", number);
 *  else                                                 snprintf(buf, sizeof
 buf, "%.6f", number);
 *  UnityPrint(buf);
 */
void UnityPrintFloat(const UNITY_DOUBLE input_number) {
  ...
  else if (number < 4294967295.9999995) /* Rounded result fits in 32 bits,
                                           "%.6f" format */
  {
    const UNITY_INT32 divisor = 1000000 / 10;
    UNITY_UINT32 integer_part = (UNITY_UINT32)number;
    UNITY_INT32 fraction_part =
        (UNITY_INT32)((number - (UNITY_DOUBLE)integer_part) * 1000000.0 + 0.5);
    /* Double precision calculation gives best performance for six rounded
     * decimal places */
    ROUND_TIES_TO_EVEN(number, fraction_part,
                       (number - (UNITY_DOUBLE)integer_part) * 1000000.0);
  ...
  } else /* Number is larger, use exponential format of 9 digits, "%.8e" */
  {
    ...
    /* Double precision calculation required for float, to produce 9 rounded
     * digits */
...
void UnityAssertGreaterNumber(const UNITY_INT shouldBeGreater,
                              const UNITY_INT actual, const char *msg,
                              const UNITY_LINE_TYPE lineNumber,
                              const UNITY_DISPLAY_STYLE_T style) {
  RETURN_IF_FAIL_OR_IGNORE;

  if (!(shouldBeGreater > actual)) {
    UnityTestResultsFailBegin(lineNumber);
    UnityPrint(UnityStrExpected);
    UnityPrintNumberByStyle(shouldBeGreater, style);
...
void UnityAssertSmallerNumber(const UNITY_INT shouldBeSmaller,
                              const UNITY_INT actual, const char *msg,
                              const UNITY_LINE_TYPE lineNumber,
                              const UNITY_DISPLAY_STYLE_T style) {
  RETURN_IF_FAIL_OR_IGNORE;

  if (!(shouldBeSmaller < actual)) {
    UnityTestResultsFailBegin(lineNumber);
    UnityPrint(UnityStrExpected);
    UnityPrintNumberByStyle(shouldBeSmaller, style);
  ...
  while (elements--) {
    ...
    if (expect_val != actual_val) {
      if (style & UNITY_DISPLAY_RANGE_UINT &&
          length < sizeof(expect_val)) { /* For UINT, remove sign extension
                                            (padding 1's) from signed type casts
                                            above */
        UNITY_INT mask = 1;
...
void UnityAssertEqualStringArray(const char **expected, const char **actual,
  ...
  do {
    ...
    if (Unity.CurrentTestFailed) {
      ...
      UnityPrintExpectedAndActualStrings((const char *)(expected[j]),
                                         (const char *)(actual[j]));
      UnityAddMsgIfSpecified(msg);
      UNITY_FAIL_AND_BAIL;
    }
